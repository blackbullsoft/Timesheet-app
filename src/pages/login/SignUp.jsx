import { View, Text, StyleSheet, Pressable, TouchableOpacity, Image, ImageBackground, Dimensions, Alert } from 'react-native'
...
export default function SignUp() {
  ...
  const handleLogin=async()=>{
    ...
    if(email.length==0 || password.length==0){
      dispatch(showToast('error', 'Required Feild', 'pls filled all required feild!'))
      return
    }
  ...
  const handleSignUp=async()=>{
    ...
    if(email.length==0 || password.length==0 || mobile.length==0 || username.length==0){
      dispatch(showToast('error', 'Required Feild', 'pls filled all required feild!'))
      return
    }
  ...
  return (
    <ImageBackground source={Background} style={{ height: "100%" }}  >
      <View style={isLogin ? styles.container1 : styles.container2}>
        {
          isLogin ? <Text style={styles.heading}>
            Login
        ...
        {
          isLogin ? <Text style={styles.subHeading}>
          ...
          isLogin ?
            <View style={styles.inputContainer}>
              ...
              <View style={styles.passwordStyle}>
                <TextInput style={[styles.inputFeild, styles.passwordInput]}
                ...
                />
                <TouchableOpacity onPress={() => setSecureText(!secureText)} style={styles.eyes}>
                  <Icon
              ...
              {
                isLogin && <Pressable style={styles.forgotContainer} onPress={()=>navigation.navigate("ForgotPassword")}>
                  <Text style={styles.forgotBtn}>
                  Forgot Password?
                  </Text>
                </Pressable>
              }
              <Pressable style={[styles.btnContainer,{backgroundColor:loading? "grey":"#0085FE"}]} onPress={()=>handleLogin()}>
            ...
            <View style={styles.inputContainer}>
              ...
              <Pressable style={[styles.btnContainer,{backgroundColor:signupLoading? "grey":"#0085FE"}]} onPress={()=>handleSignUp()}>
                <Text style={styles.btn}>
          ...
          isLogin ?
            <View style={styles.termContainer}>
              <Text style={styles.term}>Don't have an account? </Text>
                <Pressable onPress={()=>navigation.navigate('SignUp',{isLogin:false})}>
            ...
            :
            <View style={[styles.termContainer,!isLogin && styles.termContainer1]}>
              <Text style={styles.term}>By clicking sign up , </Text>
              <Text style={styles.term}>you agree to our <Text style={{ color: '#0085FE' }}>  terms of Service</Text> and <Text style={{ color: '#0085FE' }}> Privacy policy</Text></Text>
