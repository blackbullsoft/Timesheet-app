...
api=`${API_BASE_URL}/timesheet/v2/employee/entryview?startDate=${startDate}&endDate=${endDate}&userId=${userId}`
    ...
    const totalHours=events.reduce((total, entry) => total + parseFloat(entry.hours), 0);
    
    const transformedEvents = events.reduce((acc, event) => {
    const { date, username, location, eventColor, startTime, endTime, hours,employeeId,timesheet_id } = event;
    ...
    yield put({ type: 'FETCH_EVENT_LIST_SUCCESS', payload: {data:transformedEvents,totalHours} });
    ...
    yield put({ type: 'FETCH_EVENT_LIST_SUCCESS', payload: {data:[],totalHours:0} });
  }
}
